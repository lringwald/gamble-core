...
INPUT <- Sys.getenv("BM_INPUT", "output/pixel_model_inputs_BMLEH_Los1_CAPRI.rds")
...
set.seed(20260909); te <- sort(sample(nrow(X), round(nrow(X) * 0.2))); tr <- setdiff(seq_len(nrow(X)), te)
...
SDX <- apply(X, 2, sd, na.rm = TRUE); SDX[!is.finite(SDX) | SDX == 0] <- 1
...
mu_cred <- pmax(0, (p_sign[ord_mu, , drop = FALSE] - 0.5) * 2)
...
<style>
 ...
 body{font:15px/1.6 -apple-system,BlinkMacSystemFont,"Segoe UI",Helvetica,Arial,sans-serif;color:var(--ink);
      background:var(--bg);max-width:1080px;margin:2.2rem auto;padding:0 1.4rem}
 h1{font-size:1.6rem;margin:0 0 .2rem} h2{font-size:1.12rem;margin:2.1rem 0 .5rem;padding-bottom:.3rem;
   border-bottom:2px solid var(--line)} .sub{color:var(--mut);margin:0 0 1.6rem}
 table{border-collapse:collapse;width:100%;margin:.6rem 0;font-size:13.5px}
 th{text-align:left;border-bottom:2px solid var(--line);padding:.4rem .5rem;color:var(--mut);font-weight:600}
 td{border-bottom:1px solid var(--line);padding:.34rem .5rem;font-variant-numeric:tabular-nums}
 tr.warn td{background:var(--warnbg)}
 .box{border-left:4px solid var(--warnln);background:var(--warnbg);padding:.8rem 1rem;margin:1rem 0;border-radius:0 4px 4px 0}
 .ok{border-left-color:var(--ok);background:#f1f7f4}
 .kv{display:flex;flex-wrap:wrap;gap:1.6rem;margin:.6rem 0} .kv div{min-width:140px}
 .kv b{display:block;font-size:1.4rem;font-variant-numeric:tabular-nums} .kv span{color:var(--mut);font-size:12.5px}
 code{background:#f4f6f8;padding:.1rem .3rem;border-radius:3px;font-size:12.5px}
 .wrap{overflow-x:auto}
 table.heat{border-collapse:separate;border-spacing:1px;font-size:11px;width:auto}
 table.heat th.rl{text-align:right;padding:.1rem .45rem;white-space:nowrap;border:0;font-weight:500;color:var(--ink)}
 ...
 table.heat th.rl .rm{display:inline-block;font-size:8px;letter-spacing:.04em;color:#3f8f6b;
 ...
 th.rot>span{writing-mode:vertical-rl;transform:rotate(180deg);white-space:nowrap;
   font-weight:500;font-size:10.5px;color:var(--mut)}
</style>
<h1>BMLEH_Los1_CAPRI — prior module</h1>
