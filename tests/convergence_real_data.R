...
fit_arm <- function(arm) {
  future_lapply(seq_len(NCHAIN), function(ch) {
    ...
    mnlogit_rcpp_sym(
      ...
      re_mean_shift = identical(arm, "mean_shift"),
...
BLOCKS <- list(
  ...
  "lambda (HS)"= function(f) sub_cols(as_mat(f$post_kappa_pooled)),  # FE horseshoe LOCAL scales
...
summarise_arm <- function(chains, label) {
  ...
  for (nm in names(BLOCKS)) {
    ...
    if (any(vapply(lst, is.null, TRUE))) { cat(sprintf("  %-12s   -- not produced in this config\n", nm)); next }
